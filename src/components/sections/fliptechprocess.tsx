...
icon: (
  ...
  return (
    <section className="w-full px-8 py-16 text-center bg-white dark:bg-black transition-colors duration-300">
      <h2 className="text-4xl font-bold text-black dark:text-white">
        THE <span className="text-emerald-400 ">FLIP-TECH</span> PROCESS
      </h2>
      ...
      <div className="relative mt-12">
        ...
        <div className="hidden md:block absolute left-0 top-1/2 w-full h-1 bg-gray-300 dark:bg-gray-700 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={index} className="flex flex-col items-center">
                <div
                  className={`text-2xl rounded-full p-2 md:p-3 shadow-md transition-all duration-300 ${
                    isActive
                ...
                <p className="text-sm text-emerald-400 font-semibold mt-2">{step.day}</p>
                <h3 className="mt-5 font-bold text-lg text-black dark:text-white">{step.title}</h3>
                <p className=" max-w-[200px] text-sm text-gray-600 dark:text-gray-400">{step.desc}</p>
              </div>
            );
          })}
        </div>
